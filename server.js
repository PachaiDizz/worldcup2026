...
const manualMatchNo = {}; // matchNo (string) → { home, away, status }
...
app.get('/api/admin/set-score', (req, res) => {
  ...
  console.log(`⚡ Manual score: Match ${matchNo} = ${homeScore}-${awayScore} (${status || 'FINISHED'})`);
...
app.get('/api/scores', async (req, res) => {
  try {
    ...
    try {
      ...
      // API unavailable — return manual scores only
      const manualMatches = Object.entries(manualMatchNo).map(([no, s]) => ({
        id: `manual-${no}`,
        utcDate: new Date().toISOString(),
        status: s.status,
        homeTeam: { name: `Match ${no}` },
        awayTeam: { name: '' },
